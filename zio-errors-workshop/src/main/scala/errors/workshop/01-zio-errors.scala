...
object ZIOFailure extends ZIOAppDefault {
   ...
   */
  def loadConfig: IO[String, Config] = ZIO.fail("It's gone pear shaped")
...
object ZIOCatchAll extends ZIOAppDefault {
   ...
   */
  def run = loadConfig.catchAll(err => Console.printLine(err.trim))
}
...
object ZIODie extends ZIOAppDefault {
   ...
   */
  def loadConfig: IO[String, Config] = ZIO.die(new NullPointerException("It's gone pear shaped"))
...
object ZIOSandbox extends ZIOAppDefault {
   ...
   */
  def run = loadConfig.catchAllCause {
    case e: NullPointerException => Console.printLine(e.getMessage)
  }
}
...
object ZIORefinement extends ZIOAppDefault {
  ...
  def loadMyConfig: IO[ConfigNotFoundError, Config] =
    loadSomeConfig("/does/not/exist", myConfigReader).refineOrDie {
      case e: FileNotFoundException => ConfigNotFoundError(e.getMessage)
    }
